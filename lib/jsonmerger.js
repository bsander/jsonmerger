module.exports = function jsonMerger(files, options) {
  ...
  function preProcessStageConfig(config) {
    ...
    _.each(config, function (stage) {
      merged = _.merge({ _strategy: stage.config._strategy }, unwrap(_.merge(wrap(merged), wrap(stage.config), options.strategy)));
  ...
  function ignoreStrategy(a) {
    ...
    // strategy directives)
    return unwrap(_.merge(wrap(a), wrap({}), options.strategy));
  }
